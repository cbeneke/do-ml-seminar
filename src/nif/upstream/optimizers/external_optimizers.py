...
class AdaBeliefOptimizer(tf.keras.optimizers.Optimizer):
    ...
    def build(self, var_list):
        ...
        AdaBelief optimizer has 2 types of variables: momentums (m), velocities (v),
        and velocity hats (vhat, optional)
        ...
        self._momentums = []
        self._velocities = []
        self._velocity_hats = [] if self.amsgrad else None
    ...
    def update_step(self, gradient, variable, learning_rate):
        ...
        # Get momentum and velocity variables
        ...
        # Update moments
        m_t = beta_1_t * m + (1.0 - beta_1_t) * gradient
        m.assign(m_t)
        m_corr_t = m_t / (1.0 - beta_1_power)

        # Update velocities
        v_t = beta_2_t * v + (1.0 - beta_2_t) * tf.square(gradient - m_t) + epsilon_t
        v.assign(v_t)

        if self.amsgrad:
            vhat = self._velocity_hats[self._get_variable_index(variable)]
        ...
        if self._has_weight_decay:
            var_t += tf.cast(self.weight_decay, var_dtype) * variable

        variable.assign_sub(lr_t * var_t)
